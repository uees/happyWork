...
def load_product_class(file_path, unmerge=False):
    ...
    for row in range(2, excel.max_row()+1):
        ...
        excel.set_range_value(1, row, [[xuhao, new_costing_classification, costing_classification, 
                                        accounting_classification, slug, model_name, 
                                        unit, product_code, datetime.strftime(new_note,'%Y-%m-%d') if new_note else None, 
...
def load_cangku_liushui(file_path, start_row, stop_row):
    ...
    for row in range(start_row, stop_row+1):
        ...
        note, peifang_version = [excel.get_cell_value(row=row, col=col) for col in range(1, 13)]
...
def tongji_ruku(dest_filename='empty_book.xlsx'):
    ...
    for product in products:
        ...
        ws.cell(row=row, column=3).value = datetime.strftime(product.product_date,'%Y-%m-%d') if product.product_date else None
        ws.cell(row=row, column=4).value = datetime.strftime(product.wan_cheng_ri,'%Y-%m-%d') if product.wan_cheng_ri else None
