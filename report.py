...
def generate_iqc_reports(filename, end_row=None):
    ...
    for row in ws.iter_rows('B7:G{}'.format(end_row)):
        ...
        for item in qc_items:
            ...
            if item == '细度':
                ...
                if _material_name.find('A0055') >=0 or _material_name.find('A0063') >=0:
                    template_ws.cell('C{}'.format(row)).value = \
                        '%s~%s℃' % (str(180+random.randint(1, 9)), str(220-random.randint(1, 9)))
                elif _material_name.find('A0058') >=0:
                    template_ws.cell('C{}'.format(row)).value = \
                        '%s~%s℃' % (str(135+random.randint(1, 5)), str(150-random.randint(1, 5)))
                else:
                    template_ws.cell('C{}'.format(row)).value = '√'
            else:
                template_ws.cell('C{}'.format(row)).value = '√'
        ...
        new_filename = '%s-%s-%s-%s.xlsx' % (incoming_date, random.randint(1,99), material_name, supplier)
...
class Generator(object):
    ...
    def fqc_record(self, product):
        filename='%s/reports/FQC&IQC检测记录.xlsx' % self.app_path
        wb = load_workbook(filename)
        ws = wb.get_sheet_by_name('FQC')
        row = ws.max_row + 1
        ...
        for item in items:
            if item == '外观颜色' or item == '板面效果' or item == '固化性'\
                ...
                if c == 'uvw5d10' or c == 'uvw5d65' or c == 'a2' or c == 'k2'\
                        or c == 'thw5d35':
                    record.append({'col':self._get_col(item), 'value':'≤10μm'})
                elif c == 'tm3100' or c == 'ts3000' or c == 'uvs1000' or c == 'uvm1800':
                    record.append({'col':self._get_col(item), 'value':'≤20μm'})
                else:
                    record.append({'col':self._get_col(item), 'value':'≤5μm'})
            elif item == '粘度':
    ...
    def _get_product_info(self, index):
        ...
        customer, internal_name, spec, batch, amount, product_date = \
            map(null2str, [customer, internal_name, spec, batch, amount, product_date])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--create_all", action="store_true", default=False, help="初始化数据库")
