...
app_path = exe_path()
report_path = r'{}\reports'.format(app_path)
output_dir = r'{}\搜索结果'.format(app_path)

def get_all_docs():
    docs = glob.glob(r"{}\*.doc".format(report_path))  # a list
    for dir_name in os.listdir(report_path):
        dir_path = r"{}\{}".format(report_path, dir_name)
        if os.path.isdir(dir_path): 
            docs.extend(glob.glob(r"{}\*.doc".format(dir_path)))
    return docs
...
def search(batch, docs):
    ...
    if not is_find:
        ...
        with open(r'{}\log.txt'.format(output_dir), 'a') as fp:
            fp.write('批号{}的报告没找到\n'.format(batch))
...
def run():
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
    docs = get_all_docs()
    with open(r'{}\批号.txt'.format(app_path), 'r') as fp:
